...
| AI 联网搜案例 | DeepSeek Function Calling + **DDG/Tavily/Bing 三引擎**，按4角度拆分搜索词，智能过滤+相关性评分，顶部固定**可折叠 + 横滑 + 可点击跳转**案例卡片，**后端 DB 持久化全组共享** |
...
| 群聊任务清单 | ⋯ 菜单中的"📋 任务清单"，自己的可点击进 TaskChatPage |
| 团队看板（v4.1） | 项目综述 + 小组讨论方案 + 总进度条 + 任务分工（DDL 升序） |
| 主动联网搜案例（v4.1） | 任意项目可点击"🍡 让 AI 帮你搜索参考案例"按钮触发 |
...
| 联网搜索 | Function Calling 调 **DDG（优先）/ Tavily API（备用）/ Bing（末选）**，按4角度拆分搜索词，垃圾过滤+相关性评分，返回卡片（标题/摘要/host/链接） |
...
| 联网搜索 | httpx + DDG HTML / Tavily API / Bing | **DDG 优先** + Tavily 备用 + Bing 末选三引擎，智能过滤+相关性评分 |
...
│   │   ├── schedule.py
│   │   ├── group.py             # Group + 状态机 + search_results
│   │   ├── notification.py
│   │   ├── friendship.py
...
│   │   ├── groups.py            # 群组 + 6 阶段工作流 + submit-proposal + ask-ai + search-results
│   │   ├── friends.py           # 好友
...
│   │   ├── web_search.py        # DDG + Tavily + Bing 三引擎搜索
│   │   ├── ai_split.py
│   │   ├── ai_parse.py
│   │   ├── smart_assign.py
│   │   └── ddl_reminder.py
...
│   └── src/
│       ├── main.jsx
│       ├── App.jsx              # 路由 + TaskChatPage + AIChatPage/:sessionId
...
- **v4.4** (2026-06-14) — DDG+Tavily+Bing 三引擎、搜索质量过滤评分、AI 知识库自动生成、执行中调整审批
- **v4.3** (2026-06-13) — 搜索结果后端持久化、AI 多会话、子任务逾期预警、已完成项目降灰
- **v4.2** — 技能名片专业推荐、AI 任务分配三维评估
- **v4.1** — 群聊顶部精简、看板重写、主动联网搜索
- **v4.0** — 6 阶段工作流、三层任务结构、TaskChatPage
