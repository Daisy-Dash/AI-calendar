...
| AI 联网搜案例 | DeepSeek Function Calling + **Bing/DuckDuckGo 双引擎**，顶部固定**可折叠 + 横滑 + 可点击跳转**案例卡片，**后端 DB 持久化全组共享** |
...
| 联网搜索 | Function Calling 调 **Bing（优先）/ DuckDuckGo（备用）**，返回卡片（标题/摘要/host/链接） |
...
| 联网搜索 | httpx + Bing / duckduckgo_search | **Bing 优先** + DDG 备用双引擎，无需额外 API Key |
...
│   │   ├── web_search.py        # Bing + DuckDuckGo 双引擎搜索
│   │   ├── ai_split.py
│   │   ├── ai_parse.py
│   │   ├── smart_assign.py
│   │   └── ddl_reminder.py
